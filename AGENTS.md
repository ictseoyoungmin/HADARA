...
- For task workflow commands, follow `docs/TASK_WORKFLOW_COMMANDS.md`: from 0.4 onward, agents should use `task status` for next-work selection, phase checks, and next-action guidance, then reviewed `task finalize --json` / `task finalize --execute --plan-hash <hash>` as the default close path. `task next`, `task show`, `task lifecycle`, `task finish`, `task ready`, `task close`, `task audit-close`, and `task complete` were removed in 0.4.1-rc.0 and answer with redirect stubs; use finalize (dry-run or `--execute --auto`) and `task status --detail full` instead.
