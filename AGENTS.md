...
- Parallelize read-only discovery, file inspection, independent validation, package/registry metadata inspection, read-only diagnostics, and draft preparation before writes. Serialize same-file writes, evidence append, Task Capsule doc writes, Task Board writes, Project State writes, Agent Handoff writes, before-hash execute operations, `task close`, compatibility `task finalize --execute`, and release artifact/publish operations.
