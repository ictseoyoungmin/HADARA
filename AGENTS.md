...
- For task workflow commands, follow `docs/TASK_WORKFLOW_COMMANDS.md`: from 0.4 onward, agents should use `task status` for next-work selection, phase checks, and next-action guidance, then reviewed `task finalize --json` / `task finalize --execute --plan-hash <hash>` as the default close path. Use `task next`, `task lifecycle`, `task finish`, `task ready`, `task close`, and `task audit-close` only as compatibility, debugging, recovery, or command implementation surfaces.
