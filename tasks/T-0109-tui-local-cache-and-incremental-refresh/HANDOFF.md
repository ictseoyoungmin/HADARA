...
T-0109 added an internal TUI local cache service and opt-in interactive `hadara tui --cache` path. Cache records are written only under `.hadara/local/tui/read-model-cache.json`, include task index mtime/size/hash data, support full/fast/detail/none refresh modes, and remain excluded from context export. Default interactive TUI, `--no-cache`, and snapshot mode remain cache-free.
