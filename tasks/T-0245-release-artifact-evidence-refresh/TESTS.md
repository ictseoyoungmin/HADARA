...
| `npm run dev:docker-check` | Validate release artifact npm-cache and freshness hardening source changes. | Yes after code change | Passed: 92 files, 615 tests. | Command output and evidence record. |
| `npm run dev:docker-sync-build` | Refresh `dist` after release artifact and dry-run service changes. | Yes after code change | Passed: 92 files, 615 tests; `distLooksStale:false`. | Command output and evidence record. |
| `node dist/cli/main.js release artifact --execute --json --output dist-release --attach-evidence --task T-0245` | Generate and attach current release artifact evidence. | Yes | Passed; attached public report artifact for T-0245 at commit `2eff19c8ab63b635804352d2c71803226d592749`. | Command output and attached evidence artifact. |
| `node dist/cli/main.js release dry-run --json` | Confirm release dry-run readiness after artifact refresh. | Yes | Passed; `ok:true`, readiness `ready`, blockers 0, latest artifact evidence accepted. | Command output and evidence record. |
...
| Docker full check | Yes after code change | T-0245 changes release artifact and release dry-run service code. | Passed: 92 files, 615 tests. | Command output and evidence record. |
