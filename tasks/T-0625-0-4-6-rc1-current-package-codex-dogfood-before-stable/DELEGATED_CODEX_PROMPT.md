...
You are working in `/mnt/f/NowWorking/dev/hadara-046rc1-current-codex-dogfood`.
...
```bash
node /mnt/f/NowWorking/dev/hadara-046rc1-current-codex-dogfood/.hadara-install/node_modules/hadara/dist/cli/main.js
```

Act like a normal project user. Follow the generated `AGENTS.md`, `.hadara/context/HADARA_CONTEXT.md`, and `docs/HADARA_WORKFLOW.md`. Do not use any HADARA-dev internal knowledge.

Build a small but usable MVP called "Quant Battle Arena" in this repository using HADARA task capsules:

1. Use HADARA to create/inspect the recommended adoption-baseline task if the project asks for it, then close it correctly.
...
- Stop after the MVP tasks are closed or if a HADARA UX bug blocks you. Write a short `DOGFOOD_NOTES.md` in the project root summarizing what worked, what was confusing, and any blocker.
