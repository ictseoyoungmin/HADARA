...
| README claims rc.1 is installable before npm publish. | Public users could copy a non-existent install command. | Medium | README distinguishes current published rc.0 before publish from rc.1 install after T-0301 helper verification. | Mitigated |
...
| Actual npm publish mutates the registry. | Irreversible package version publication. | Certain if executed | This capsule does not publish; helper still requires npm auth, exact version absence, npm publish dry-run, and interactive `publish` confirmation. | Operator-owned |
