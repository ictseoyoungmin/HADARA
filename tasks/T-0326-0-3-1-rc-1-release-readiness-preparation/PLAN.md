...
| 4 | Run Docker build/full validation and refresh release evidence without publish mutation. | In Progress | Docker sync-build passed on rerun after one timeout; release artifact, package smoke, clean-checkout smoke, strict gate, release dry-run, publish dry-run, and final `git diff --check` pending after source checkpoint commit. |
| 5 | Finalize capsule/shared docs, finish, ready, close, audit, and commit T-0326. | Pending | Lifecycle commands after validation and final doc updates. |
