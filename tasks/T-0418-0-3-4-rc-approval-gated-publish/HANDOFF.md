...
| The first publish-env preparation attempt was interrupted and left a broken clone. | A stale clone can have D/?? git state and missing `dist`. | It was deleted/recreated; before publishing, rerun `git status --short` and `node dist/cli/main.js version --json` in `/root/hadara-publish`. |
