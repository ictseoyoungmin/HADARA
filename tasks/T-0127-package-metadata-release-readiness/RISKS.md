...
| Bootstrap metadata gate breaks intentionally when moving to `0.1.0-rc.N` and `private: false`. | Make the readiness decision mode-aware: bootstrap mode is valid now; release-candidate mode must require `private: false`, `files`, `LICENSE`, and package-smoke evidence. |
| `docs/TEST_STRATEGY.md` marker checks become too fragile as release planning grows. | Treat marker-based checks as temporary and move T-0128+ readiness state to a dedicated `docs/RELEASE_READINESS.md` or typed `docs/release-readiness.json` source. |
