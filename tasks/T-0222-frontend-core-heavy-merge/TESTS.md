...
| `node --input-type=module -e "...detail evidence/timeline smoke..."` | Check built route payloads for evidence labels and projected timeline summaries. | Yes | Passed: T-0223 first evidence record returned `kind: command-log`, `result: passed`, `visibility: public`; projected timeline returned data-row summaries for handoff/next work. | Command output observed 2026-06-02. |
| `npm run dashboard:visual:docker` | Run Playwright/axe visual gate after served bundle rebuild. | Yes | Passed | All projection-ready/detail/stale/refreshing/missing/offline/degraded visual/a11y checks passed. |
...
| Evidence label smoke | Yes | Evidence cards must not show `unknown` when canonical kind/result exist. | Passed: frontend normalizes and displays evidence `kind`, `result`, and `visibility`; built route payload exposes `command-log/passed/public`. | Docker sync-build and built route smoke. |
