...
| AC-6 | Release artifact evidence for rc3 is refreshed. | Met | Release artifact evidence passed for `hadara-0.2.0-rc.3` tarball/checksum/manifest. |
| AC-7 | Release readiness reports no blockers without mutation. | Met | Release dry-run returned ready; release publish dry-run returned ok:true with no publish/GitHub/Docker mutation. |
