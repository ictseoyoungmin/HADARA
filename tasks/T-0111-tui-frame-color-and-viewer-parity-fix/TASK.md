...
- Optimize fast cache validation so unchanged project-level source signals reuse cached hashes after matching `mtimeMs` and `size`, avoiding reads of `TASK_BOARD.md`, handoff, active-run, selected task, and selected evidence files on cache hits.
