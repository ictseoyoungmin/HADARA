...
| `src/tui/state.ts` | Modify | Keep task cursor/page movement aligned and let numeric input through while Tasks search is active. |
...
| `tests/unit/tui-state.test.ts` | Modify | Cover height-derived task window behavior and numeric search input. |
