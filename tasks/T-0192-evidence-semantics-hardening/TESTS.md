...
|---|---|---|---|---|
| npm test | Run the default project test suite. | Yes | Passed in Docker | Included in `npm run dev:docker-sync-build`; 79 files / 549 tests passed. |
...
| Built CLI version smoke | Yes | Confirms Docker build refreshed `/workspace/dist`. | Passed | `version --verbose --json` returned `ok:true`, `distLooksStale:false`. |
