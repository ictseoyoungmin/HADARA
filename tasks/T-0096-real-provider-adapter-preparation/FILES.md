...
| Path | Action | Reason |
|---|---|---|
| `src/providers/provider-preparation.ts` | Added/Updated | Defines provider config normalization and provider call report helpers without network calls or secret loading; now denies unknown input fields and asserts schema-valid outputs. |
...
| `src/core/events.ts` | Updated | Adds optional schema assertion for structured HADARA events. |
| `tests/unit/events.test.ts` | Updated | Covers event schema assertion success and failure behavior. |
...
| `docs/V1_0_IMPLEMENTATION_SCHEMAS.md` | Updated | Records current provider preparation status and deferred ActionIntent/audit contract sketch. |
