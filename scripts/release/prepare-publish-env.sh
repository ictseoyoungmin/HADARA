...
#    For the current stable 0.4.2 path:
...
# git add -A && git commit -m "T-0545 0.4.2 stable release readiness and publish preparation"
#
#
# What it does:
...
#   bash scripts/release/prepare-publish-env.sh T-0545
...
if [[ -z "$TASK_ID" ]]; then
  echo "TASK_ID is required (the release Task Capsule id, e.g. T-0545)."
  usage
  exit 1
fi
