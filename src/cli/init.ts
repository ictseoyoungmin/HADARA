...
export function initProject(projectRoot: string, profile: InitProfile = 'minimal'): void {
  ...
  writeFileIfMissing(path.join(projectRoot, 'AGENTS.md'), createAgentsDoc());
...
7. Read the active Task Capsule files before implementation.
8. Read project-specific docs listed in the Required Reading table below.
...
| \`docs/IMPLEMENTATION_SOP.md\` | Every session | Local HADARA workflow rules. |
| \`docs/DEVELOPMENT_SLICES.md\` | Starting or completing slices | Roadmap ordering and completion evidence. |
...
|---|---|---|---|
| \`minimal\` | Core protocol docs only | General projects adopting Task Capsules and handoff discipline. | No release, security-smoke, MCP, provider, dashboard, or Hermes assumptions. |
...
4. Keep authoritative per-task evidence in Task Capsules and state tracking in \`docs/TASK_BOARD.md\` and \`docs/DEVELOPMENT_SLICES.md\`.
`;
}
...
function createAgentsDoc(): string {
  return `# AGENTS
...
8. Project-specific specs, contracts, or roadmap documents listed in \`docs/IMPLEMENTATION_SOP.md\`
...
- Respect prerequisite order in \`docs/DEVELOPMENT_SLICES.md\`.
