...
Use \`evidence add-command\` only when recording an already-run result supplied by the operator. It does not execute shell commands. Use \`--artifact-file\` when a sanitized UTF-8 command or release report is part of the claim; the shared writer policy-checks it, records exact \`sha256\`/\`byteLength\`, includes those bytes in the evidence fingerprint, and binds the report into canonical evidence \`artifacts[]\`. Evidence lint fails on missing or changed bound bytes, while historical refs without byte metadata remain compatible. Same-key retries must present the same artifact bytes. Use \`evidence list\` to find durable evidence ids for docs and resolution markers.
...
| \`evidence add-command\` | Write | Appends operator-supplied command-log evidence. It does not execute shell commands or capture stdout/stderr; optional \`--artifact-file\` binds exact sanitized bytes with \`sha256\`/\`byteLength\`, optional \`--category\`/\`--outcome\`/\`--resolves\`/\`--supersedes\` enrich v2 metadata, result/outcome mismatches and changed same-key artifact bytes are rejected, and JSON responses expose \`evidence.appendLock\` wait diagnostics. |
...
| \`task close\` | Executes by default; \`--dry-run\` is read-only; reviewed execute uses \`--plan-hash\` | Default agent close path. Rechecks the current plan, applies reviewed guarded writes before proof, revalidates actual filesystem state, records readiness evidence and close proof, stops on blockers, and succeeds only after final audit is \`closed-valid\`. |
...
- \`evidence add-command\` records an operator-supplied command result; it does not run the command. \`--artifact-file\` binds a sanitized UTF-8 report with exact \`sha256\`/\`byteLength\` metadata, includes those bytes in the v2 fingerprint, and makes lint fail closed if the bound file is missing or changed; historical refs without byte metadata remain compatible. \`--category\` and \`--outcome\` set persisted v2 metadata explicitly, while \`--result\` remains the legacy-compatible command result. When both are supplied, \`--result\` must match \`--outcome\` for \`passed\`, \`failed\`, \`blocked\`, and \`unknown\`; \`recorded\` and \`not-applicable\` require \`--result unknown\` or no explicit \`--result\`. \`--resolves\` and \`--supersedes\` append exact v2 resolution tags from passed or recorded follow-up evidence. \`--idempotency-key\` is optional; when supplied, same-key repeats return the existing record only when incoming artifact bytes match, otherwise the append fails with \`EVIDENCE_IDEMPOTENCY_ARTIFACT_CONFLICT\`. Evidence append responses include \`evidence.appendLock\`; if \`contended\` is true, another process held the task-scoped append lock before this write.
