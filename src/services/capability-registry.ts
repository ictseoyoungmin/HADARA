...
export const HADARA_COMMAND_REGISTRY: CommandRegistryEntry[] = [
  ...
  {
    id: 'init',
    command: 'hadara init [--project <path>] [--preset minimal|standard|governed] [--adopt --execute --plan-hash <hash>] [--json]',
    ...
    examples: [
      example('Initialize the governed preset', 'hadara init --preset governed --json', 'When starting a new empty HADARA project with governance documentation.'),
      example('Review standard brownfield adoption', 'hadara init --preset standard --json', 'In an existing project, returns a zero-write adoption plan before any scaffold mutation.')
    ],
  ...
  {
    id: 'validation.run',
    command: 'hadara validation run --task <task-id> --check <name> [--timeout-ms <ms>] [--update-task] [--show-raw-output] [--show-raw-argv] [--compat v1] [--direct-result passed|failed|blocked] [--direct-summary <text>] [--resolves <id>] [--supersedes <id>] -- <command...>',
    summary: 'Run a validation command, append execution evidence, refresh evidence projection, auto-resolve earlier failed attempts for the same check identity, and optionally sync the matching TASK.md Validation row with --update-task.',
    canonical: true,
    appearsInDefaultHelp: true,
    family: 'capsule-lifecycle',
    scope: 'evidence',
