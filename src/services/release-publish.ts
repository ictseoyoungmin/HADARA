...
export function createReleasePublishReport(options: ReleasePublishOptions): ReleasePublishReport {
  ...
  const checks: ReleasePublishReport['checks'] = [
    ...
    },
    {
      ...
      summary:
        metadataPublishable ? 'Package metadata is in release-candidate mode.' : 'Package metadata must be 0.x.0-rc.N with private false before publish/deploy readiness can pass.'
    },
    {
