...
/**
 * Minimal generic-governed graph (docx section 7.2/7.3), scoped to the facts Phase A
 * (T-0659) currently produces: project.activeWork and project.nextWork. It reproduces
 * task-selection-status-v2's top-level routing decision (work available vs idle), not
 * its full handoff/board/slice fallback chain — that chain remains Phase D/registry work.
 */
export const genericGovernedGraph: StatusGraph = {
  ...
  nodes: [
    { id: 'current-state', type: 'source', adapter: 'project-current-state' },
