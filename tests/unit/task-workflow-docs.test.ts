...
describe('task workflow command semantics docs', () => {
  ...
  it('keeps read-only, dry-run, write, and ok semantics visible to operators and JSON consumers', () => {
    ...
    expect(workflow).toContain('| `hadara evidence add-command --task T-XXXX --summary "..." --result passed [--outcome <outcome>] [--category <category>] [--artifact-file <path>] [--resolves <id>] [--supersedes <id>] [--idempotency-key <key>] --json` | Record command-log evidence supplied by the operator; optionally bind a sanitized UTF-8 report with exact `sha256`/`byteLength` into canonical `artifacts[]`. | Write command. | Yes, appends capsule evidence and copies the artifact through the shared public-artifact policy unless an explicit idempotency key already exists; bound bytes are included in the fingerprint. |');
