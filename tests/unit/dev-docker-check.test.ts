...
describe('dev docker-check report', () => {
  ...
  it('blocks dist sync when workspace dist changes after the reviewed before-hash but before the sync step runs', () => {
    const root = tempProject();
    const beforeHash = hashDist(root);
    const runner: DevDockerCommandRunner = {
      run(_command, args) {
        ...
        if (classifyScript(script) === 'npm-ci') {
          // Simulate a concurrent actor changing workspace dist mid-run,
          // after the reviewed before-hash was captured but before the
          // destructive dist-sync step is reached.
          fs.writeFileSync(path.join(root, 'dist', 'cli', 'main.js'), 'raced\n', 'utf8');
        }
    ...
    expect(fs.readFileSync(path.join(root, 'dist', 'cli', 'main.js'), 'utf8')).toBe('raced\n');
    expect(validateSchema('hadara.dev.docker_check.v1', report).ok).toBe(true);
  });
