...
describe('Task Capsule scaffold frames', () => {
  it('creates the 0.4 four-file Task Capsule scaffold', () => {
    ...
    expect(taskMarkdown).toContain('| Path / Source | Type | Authority | State | Notes | Hash |');
    ...
    expect(taskMarkdown).toContain('| Check | Gate | Result | Evidence |');
    expect(taskMarkdown).toContain('## Changes');
    expect(taskMarkdown).toContain('| Area | Summary | Evidence |');
