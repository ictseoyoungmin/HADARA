...
- TUI local cache exists as an internal TypeScript service in `src/tui/cache.ts`; it writes `hadara.tui.cache.v1` records only under `.hadara/local/tui/read-model-cache.json`, indexes task summaries with mtime/size/SHA-256 invalidation data, supports full/fast/detail/none refresh modes, keeps selected-detail refresh scoped to the selected cached capsule, and remains excluded from context export.
