...
- Production TUI public CLI entry point is not implemented; only mockups, design planning, the internal read-model aggregator, mockup-derived rendering modules, snapshot renderer, pure interaction state, and internal raw terminal shell exist. Future TUI work must remain read-only until an explicit Task Capsule expands scope.
