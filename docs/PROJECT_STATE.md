...
- TUI interactive state exists as an internal pure TypeScript module in `src/tui/state.ts`; it supports panel switching, task selection/search, detail document tab selection, local scroll offsets, refresh requests, quit requests, and renderer/read-model option mapping without terminal raw mode, CLI entry point, cache writes, shell execution, provider calls, MCP calls, or Task Capsule mutation.
...
- Production TUI interactive shell and CLI entry point are not implemented; only mockups, design planning, the internal read-model aggregator, mockup-derived rendering modules, snapshot renderer, and pure interaction state exist. Future TUI work must remain read-only until an explicit Task Capsule expands scope.
