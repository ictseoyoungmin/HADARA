...
- Production TUI public CLI entry is implemented as a read-only local work console through `hadara tui` and `hadara tui --snapshot`; optional TUI cache persistence is local-only under `.hadara/local/tui/`; TUI task/evidence/handoff writes, shell execution, provider calls, MCP calls, dashboard/server behavior, and release/package behavior remain deferred.
