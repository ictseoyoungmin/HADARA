...
- Release-gate reporting now includes a read-only release checklist for package bin metadata, validation scripts, Node 22 policy, CI clean install behavior, clean-checkout smoke planning, generated artifact boundaries, and operational debt. Readiness failures are warning-only in advisory mode and blocking in strict mode.
