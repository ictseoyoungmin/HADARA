...
- TUI raw terminal shell exists as an internal TypeScript module in `src/tui/terminal.ts`; it supports decoded keyboard input, injected input/output streams, redraws through the snapshot renderer, refresh/detail-refresh effect handling, and clean shutdown/raw-mode restoration without cache writes, shell execution, provider calls, MCP calls, evidence writes, handoff updates, or Task Capsule mutation.
- A public read-only TUI CLI entry point exists as `hadara tui`, with `hadara tui --snapshot` for non-interactive smoke rendering. Non-TTY interactive runs are refused with guidance to use snapshot mode; the command remains local/read-only and introduces no task/evidence/handoff mutation, cache writes, shell execution, provider calls, MCP calls, dashboard/server behavior, or release/package behavior.
...
- Production TUI public CLI entry is implemented as a read-only local work console through `hadara tui` and `hadara tui --snapshot`; TUI writes, cache persistence, shell execution, provider calls, MCP calls, dashboard/server behavior, and release/package behavior remain deferred.
