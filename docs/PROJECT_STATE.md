...
- Current production TUI responsiveness still differs from the mockup: measured in `hadara-cli-test` against `/workspace`, startup/full/detail paths take roughly 18-24s because `createTuiReadModel()` synchronously rebuilds the full aggregate, including expensive operations status, debt, and release-gate reads. The mockup used fixture/state-cache data, async render timers, parallel CLI reads, fast partial refresh, and selected-detail-only refresh. A follow-up capsule should refactor TUI loading/read-model boundaries before release work.
