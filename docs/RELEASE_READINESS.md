...
- Current published stable npm release target is `0.4.0` after the T-0490 approval-gated publish completes.
- Current stable npm publish status: T-0490 prepared stable `hadara@0.4.0` source/readiness only; exact npm `hadara@0.4.0` lookup returned not found before publish, and the operator must verify `latest=0.4.0` after the approval-gated publish. Previous stable `hadara@0.3.3` is published on npm through T-0406 and recycled from installed consumer paths through T-0407.
