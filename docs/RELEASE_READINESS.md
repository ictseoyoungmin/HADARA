...
- Current 0.5.0 release-candidate source/preparation status: T-0634 through T-0649 completed, validated, and published the `0.5.0-rc.0` status-ingress release candidate on npm `next`.
- Current 0.5.0-rc.1 source status: T-0658 hardened task close recovery action boundaries and blocked project-status routing precedence. T-0659 through T-0661 added a declarative-DAG-inspired Fact model and DAG evaluator (internal, unused by any command) plus a project-level `continuation` field so a closing session's declared next step survives when `nextWork` is null. T-0662 implemented `anyOf` in the schema validator, previously a silent no-op across six schemas. T-0663 ran a two-session delegated Claude dogfood in a fresh external project and found real gaps in the continuation work. T-0664 fixed the root cause (bootstrap `nextWork` retirement was title-matched instead of origin-based, so it could get stuck permanently) and re-verified the dogfood's other finding was resolved as a consequence. T-0665 fixed a continuation-promotion contradiction (a "no further work" HANDOFF step was classified `actionable`, offering it as a `hadara task create` title) and bumped the version to `0.5.0-rc.1`; T-0666 promoted the validation baseline and current release state to that candidate.
- Current 0.5.0-rc.1 release-readiness and publish status: T-0667 recycled package smoke, clean-checkout smoke, release artifact, strict release gate, release dry-run, and publish dry-run evidence from a freshly pulled `node:22-bookworm` Docker image and newly recreated `hadara-dev` container. Package smoke passed with `--timeout 300` after two recorded default-timeout failures; clean-checkout smoke passed; release artifact passed from a clean ext4 clone with tarball hash `sha256:4c8157975530b0228953cd302e17e92b9faef4065c8e5ebb12dcc151fe086a1d` and manifest hash `sha256:7579aabf4c42535978efa8b84cb09992a263edc7d89805cd8763c5119c623642`; strict release gate passed; release dry-run returned `ready` with blockers 0; publish dry-run returned `ok:true` with token warnings only and no mutation. The operator then published `hadara@0.5.0-rc.1` to npm, and T-0668 recorded workspace evidence after npm registry verification returned `version=0.5.0-rc.1`, `next=0.5.0-rc.1`, and `latest=0.4.6`. T-0668 also created the GitHub Release note artifact for `v0.5.0-rc.1` and passed installed-package recycle from `hadara@next` expected `0.5.0-rc.1`. T-0669 records the operator-published public GitHub Release `v0.5.0-rc.1`, workspace verification that the release URL returns HTTP 200, and Docker installed-package dogfood from public `hadara@next` across fresh `basic`, `standard`, and `governed` projects plus package recycle.
- Planned 0.5.0-rc.2 scope: rc.2 keeps the current `0.5.0-rc.1` product line and prioritizes reviewer remediation plus HADARA-dev-only developer surfaces before any broader redesign resumes. The first active RC2 reduction targets are `operational debt`, release/package/readiness, and other development-only command or verification surfaces that do not define the ordinary end-user Capsule lifecycle. Do not treat the earlier DAG/status redesign line as the active rc.2 path unless a later capsule explicitly restores that decision across shared docs. Do not start rc.2 release readiness until continuity semantics are aligned, the RC2 developer-surface boundary is documented, fresh-session dogfood is rerun on current HEAD, and the validation baseline is deliberately promoted beyond the current T-0678 rollup.
