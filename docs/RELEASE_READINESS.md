...
- Current stable 0.4.3 publish-preparation status: T-0570 observed `npm view hadara@0.4.3 version` returning no published match before source preparation, updates the stable release note artifacts, and leaves actual npm/GitHub publication to `scripts/release/prepare-publish-env.sh T-0570` followed by the operator-controlled `scripts/release/manual-publish-rc.sh T-0570 --execute` path from a clean ext4 clone.
