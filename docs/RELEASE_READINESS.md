...
- Published prerelease: `0.5.0-rc.4` is public on npm `next`; stable `latest=0.4.6` remains unchanged.
...
- Current release target: regenerate and readiness-validate RC5 from the current source, then hand off exact bytes for separately approved npm/GitHub publication and public recycle. No RC5 external publication has occurred in this capsule.
- Current prerelease npm status: published `next=0.5.0-rc.4`; stable `latest=0.4.6` remains unchanged. RC5 is the current unpublished source candidate.
- Current prerelease GitHub status: `v0.5.0-rc.4` remains public with `isPrerelease=true`, `isDraft=false`, and its retained three assets. RC5 GitHub publication is deferred to a separate operator capsule.
