...
- Current 0.5.0-rc.1 release-readiness status: T-0667 is recycling package smoke, clean-checkout smoke, release artifact, strict release gate, release dry-run, and publish dry-run evidence from a freshly pulled `node:22-bookworm` Docker image and newly recreated `hadara-dev` container. Post-publish recycle should install `hadara@next` and verify expected version `0.5.0-rc.1`.
