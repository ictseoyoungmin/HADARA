...
Every successful npm mutation writes `<version>-npm-publication-report.json`, regardless of whether a GitHub draft was requested. If GitHub publication also succeeds, the helper adds `<version>-operator-publication-report.json`. If npm publication succeeds but the GitHub step is interrupted—or GitHub is requested later—do not rerun npm publish. Keep the exact retained artifact bytes and invoke `scripts/release/manual-publish-rc.sh <task> --github-only --retained-artifact-dir <retained-artifact-dir> --github-repo <owner/name> --git-remote-url <remote>`. The helper treats the prior npm report's registry and dist-tag as destination authority, rejects explicit mismatches before GitHub mutation, verifies package bytes/checksum/source release-input hash/registry version, and records the GitHub repository and git remote in the final operator report.
...
- Published prerelease: `0.5.0-rc.5` is public on npm `next`; stable `latest=0.4.6` remains unchanged. GitHub `v0.5.0-rc.5` is public with `isPrerelease=true` and the retained tarball/checksum/manifest assets.
- Current source version: `0.5.0-rc.5` package metadata after T-0776 evidence-binding hardening.
- Current version is `0.5.0-rc.5` as an unpublished source candidate. T-0766 reconciled fresh Init v1 scaffolds and protocol doctor now reports zero warnings across all three presets. T-0775 introduced the evidence artifact-binding and operator publication-report contracts; T-0776 hardens them with exact bytes, integrity lint, idempotency conflict detection, corrected helper attachment, and observed dist-tag truth.
- Current release target: T-0778 RC5 publication and public consumer recycle are complete. The next decision is separate stable-promotion review; npm `latest` remains unchanged at `0.4.6`.
- Current prerelease npm status: published `next=0.5.0-rc.5`; stable `latest=0.4.6` remains unchanged. Verified with npm registry metadata on 2026-08-12.
- Current prerelease GitHub status: `v0.5.0-rc.5` is public with `isPrerelease=true`, `isDraft=false`, and exactly three assets whose observed SHA-256 digests match the retained RC5 bytes. Release URL: `https://github.com/ictseoyoungmin/HADARA/releases/tag/v0.5.0-rc.5`.
- RC5 readiness status: T-0777 generated the exact `0.5.0-rc.5` tarball/checksum/manifest from source commit `e3ac410e`; package smoke, clean-checkout smoke, strict release gate, release dry-run, and publish dry-run passed with no external mutation.
- RC5 operator artifact locator: `$HADARA_RELEASE_WORKSPACE/0.5.0-rc.5/` containing `hadara-0.5.0-rc.5.tgz`, `hadara-0.5.0-rc.5.tgz.sha256`, and `hadara-0.5.0-rc.5.tgz.manifest.json`. Tarball SHA-256 is `e6e740773f0db1df1716436b45b2d68e48e3874deabf002f439c4e2b1fd20df6`; checksum and manifest file hashes are `e3a903cca75160585e22b1ee138e0e80abb5a7e389219694aa38020d31275d17` and `2c9f4bcda84704bbb0c5e8bcd15bf9a90a8214e7e5fe8281f436127fc39b6b22`. T-0778 npm publication and GitHub asset parity are recorded; the same retained bytes remain the provenance reference for public recycle.
- RC5 public lifecycle acceptance: T-0778 ran Codex CLI against public `hadara@next` in a disposable consumer. Init/doctor, substantive validation/evidence, close dry-run, real close execute (`closed-valid`, terminal), zero-write post-close retry, and fresh idle status with no stale continuation passed. Sanitized report is byte-bound in `ev:T-0778:94dc7fb15ce34d7c9d273cce`.
- RC4 public recycle status: T-0772 verified registry version/dist-tags, isolated public install, installed version, 57 command ids, lifecycle help, init, task creation/status ingress, context smoke, and cleanup. Its close step was intentionally a dry-run surface check; actual public close-execute, `closed-valid`, idempotent retry, and fresh-status acceptance remain T-0774 scope.
- Evidence contract boundary: operator publication and public lifecycle reports must be sanitized structured artifacts bound directly through `evidence add-command --artifact-file`; summary-only assertions are insufficient for a release-readiness basis. T-0776 makes the binding byte-verifiable and fail-closed on mutation or same-key conflict. T-0777 regenerates RC5 evidence from the current source; no retained RC4 bytes may be reused for RC5.
