...
- Current 0.4.6-rc.1 readiness: focused current-state/evidence/validation tests passed for rc.0; installed rc.0 dogfood completed; task-create serialization, docs-add workflow, and Docker fast sync-build checks passed; T-0620 retargets package metadata, release notes, readiness docs, README release status, and the GitHub prerelease note artifact to `0.4.6-rc.1`. npm publish, GitHub Release publication, token loading, and installed-package recycle remain operator-controlled follow-up actions.
