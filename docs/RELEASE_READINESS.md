...
- Current 0.4.4 release-candidate source-preparation status: T-0572 defined the external-repository validation plan; T-0573 through T-0577 ran delegated/external dogfood across basic, standard, and governed profiles; T-0575 and T-0578 fixed the release-candidate UX issues found by those runs; T-0579 retargeted source metadata, README release status, release notes, readiness docs, helper examples, and the GitHub prerelease note artifact to `0.4.4-rc.0`.
- Current 0.4.4 release-candidate readiness: R1 basic-profile delegated dogfood closed 5 capsules with installed `hadara@0.4.3`; R2 standard-profile external validation completed ordinary lifecycle checks and documented host spawn fallback as non-blocking; R3 Claude Code independently completed 8 governed-profile capsules and the reviewer separated 0.4.3-only findings from current candidate behavior. T-0578 then fixed the remaining pre-release UX issues selected from R3 before this readiness capsule.
...
- Current source version is `0.4.4-rc.0`.
- Current release target is release-candidate `0.4.4-rc.0`.
- Current release-candidate npm publish target is `0.4.4-rc.0` on dist-tag `next`.
- Current published stable npm release target is `0.4.3`.
...
- Current 0.4.4 stable promotion decision: T-0581 rechecked the published RC after T-0580 and found no stable-blocking issue. npm still resolves `hadara@0.4.4-rc.0` with `latest=0.4.3` and `next=0.4.4-rc.0`; GitHub Release `v0.4.4-rc.0` remains public prerelease; installed-package recycle from `hadara@next` expected `0.4.4-rc.0` passed; docs doctor remained healthy and clean. Decision: proceed to a separate stable `0.4.4` source/release preparation capsule.
