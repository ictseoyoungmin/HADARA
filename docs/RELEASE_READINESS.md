...
- Current 0.5.0 release-candidate source/preparation status: T-0634 through T-0647 completed the status-first 0.5.0 implementation line, including project status v2, task-selection status v2, selected-task cockpit guidance, and public `session start` removal from current routing/guidance. T-0648 retargets source metadata, release notes, README release status, readiness docs, and GitHub prerelease note artifact to `hadara@0.5.0-rc.0`.
- Current 0.5.0 release-candidate readiness: source validation covers TypeScript build, focused status/session workflow tests, context-routing status ingress smoke, package-smoke dry-run, strict release gate, and T-0649 full-suite regression cleanup before operator npm/GitHub publication. T-0649 direct full suite passed with 154 test files and 1132 tests after the validation wrapper timed out at 120s. `hadara@0.5.0-rc.0` was subsequently published to npm dist-tag `next` (per npm registry: 89 downloads in the 7 days after publish).
- Current 0.5.0-rc.1 status: T-0658 hardened task close recovery action boundaries and blocked project-status routing precedence. T-0659 through T-0661 added a declarative-DAG-inspired Fact model and DAG evaluator (internal, unused by any command) plus a project-level `continuation` field so a closing session's declared next step survives when `nextWork` is null. T-0662 implemented `anyOf` in the schema validator, previously a silent no-op across six schemas. T-0663 ran a two-session delegated Claude dogfood in a fresh external project and found real gaps in the continuation work. T-0664 fixed the root cause (bootstrap `nextWork` retirement was title-matched instead of origin-based, so it could get stuck permanently) and re-verified the dogfood's other finding was resolved as a consequence. T-0665 fixed a continuation-promotion contradiction (a "no further work" HANDOFF step was classified `actionable`, offering it as a `hadara task create` title) and bumped the version to `0.5.0-rc.1`. Post-publish recycle should install `hadara@next` and verify expected version `0.5.0-rc.1`. Strict release gate has not been rerun since the T-0649 baseline; do not treat this line as stable-ready without rerunning it.
