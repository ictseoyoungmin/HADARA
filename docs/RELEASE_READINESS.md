...
- Current source version is `0.3.0-rc.1`.
- Current published npm release candidate is `0.3.0-rc.0`.
- Previous published release candidate is `0.2.0-rc.3`.
...
- T-0301 prepares `hadara@0.3.0-rc.1` for operator npm publish with feature-focused release notes, README/release-readiness alignment, release capsule/version guardrails, and a dry-run cleanup path so a fresh `/tmp` publish clone can run the dry-run helper and then `--execute` without manual cleanup.
...
- Current operator publish path: commit T-0301 readiness, clone the committed repository into `/tmp/hadara-publish`, run `npm ci`, run `npm run check`, authenticate with `npm login --registry=https://registry.npmjs.org`, verify `npm whoami --registry=https://registry.npmjs.org`, run `bash scripts/release/manual-publish-rc.sh T-0301`, then run `bash scripts/release/manual-publish-rc.sh T-0301 --execute` and type `publish` only after the helper's own checks pass.
...
- Version policy: first release-candidate target was `0.1.0-rc.0`; the current source publish-candidate target is `0.3.0-rc.1`. Stable release targets remain approval-gated and must be set in a later release capsule.
