...
- Current version is `0.3.4-rc.0`.
- Current source version is `0.3.4-rc.0`.
- Current release target is release candidate `0.3.4-rc.0`.
- Current published stable npm release target is `0.3.3`.
- Current stable npm publish status: `hadara@0.3.3` is published on npm through T-0406 and recycled from installed consumer paths through T-0407. Registry verification returned `0.3.3`, dist-tags verified `latest=0.3.3` and `next=0.3.3-rc.0`, temporary-prefix `hadara@latest` install passed, installed bin reported `packageVersion=0.3.3`, governed init/task lifecycle/finalize/context/cache/session-start smokes passed in a disposable project, and temp cleanup completed. T-0340 previously published `hadara@0.3.2`; T-0316 previously published `hadara@0.3.0`.
- Current release-candidate status: `hadara@0.3.4-rc.0` is published on npm with dist-tag `next` through T-0418 and has installed-package consumer proof through T-0422. npm registry checks returned `0.3.4-rc.0`, dist-tags verified `next=0.3.4-rc.0` and `latest=0.3.3`, isolated `hadara@next` install reported `packageVersion:"0.3.4-rc.0"`, installed help lifecycle, fresh init, task-scoped Session Start, task lifecycle, finalize dry-run report, context pack, context slice, and temp cleanup passed. Stable `0.3.4` has not been decided.
- Previous published npm release candidate before the current source target is `0.3.3-rc.0`.
...
- T-0417 prepared `hadara@0.3.4-rc.0` source/readiness; T-0418 completed approval-gated npm publish to `next`; T-0422 completed the reviewer-requested installed-package acceptance proof from consumer paths. T-0422 also found that the published `hadara package recycle --execute` helper is not fully clean for this RC because its extra installed `context graph --json` smoke failed and the helper environment created stray source-workspace smoke capsules before cleanup. Treat this as stable-decision input, not as a stable `0.3.4` approval.
