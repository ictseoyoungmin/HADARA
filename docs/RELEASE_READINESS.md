...
- Current 0.5.0 release-candidate readiness: source validation covers TypeScript build, focused status/session workflow tests, context-routing status ingress smoke, package-smoke dry-run, and strict release gate before operator npm/GitHub publication. Post-publish recycle should install `hadara@next` and verify expected version `0.5.0-rc.0`.
