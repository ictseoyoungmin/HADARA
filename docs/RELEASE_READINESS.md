...
- Current 0.5.0-rc.1 release-readiness status: T-0667 recycled package smoke, clean-checkout smoke, release artifact, strict release gate, release dry-run, and publish dry-run evidence from a freshly pulled `node:22-bookworm` Docker image and newly recreated `hadara-dev` container. Package smoke passed with `--timeout 300` after two recorded default-timeout failures; clean-checkout smoke passed; release artifact passed from a clean ext4 clone with tarball hash `sha256:4c8157975530b0228953cd302e17e92b9faef4065c8e5ebb12dcc151fe086a1d` and manifest hash `sha256:7579aabf4c42535978efa8b84cb09992a263edc7d89805cd8763c5119c623642`; strict release gate passed; release dry-run returned `ready` with blockers 0; publish dry-run returned `ok:true` with token warnings only and no mutation. npm registry read-only verification still reports `next=0.5.0-rc.0` and `latest=0.4.6`; post-publish recycle should install `hadara@next` and verify expected version `0.5.0-rc.1`.
...
- Current release-candidate npm publish target is `0.5.0-rc.1` on dist-tag `next`, superseding the already-published `0.5.0-rc.0` (npm does not allow republishing the same version).
