...
- Current source version: `0.5.0-rc.4` package metadata with post-RC4 structural source changes from T-0775.
- Current version is `0.5.0-rc.4` for the published RC4 package; T-0766 reconciled fresh Init v1 scaffolds and protocol doctor now reports zero warnings across all three presets. T-0775 introduced the evidence artifact-binding and operator publication-report contracts; T-0776 hardens them with exact bytes, integrity lint, idempotency conflict detection, corrected helper attachment, and observed dist-tag truth.
- Current release target: RC4 exact-artifact retention, npm/GitHub publication, and public package recycle are complete, but the retained RC4 release-input identity is invalid for promotion because T-0775 changes source/runtime evidence behavior; regenerate RC5 before any stable decision.
- Current prerelease npm status: published `next=0.5.0-rc.4`; stable `latest=0.4.6` remains unchanged. T-0772 verified registry metadata and installed-package recycle from public `hadara@next`.
- Current prerelease GitHub status: `v0.5.0-rc.4` is public with `isPrerelease=true`, `isDraft=false`, and the retained tarball, checksum, and manifest assets attached. GitHub asset digests match the retained T-0770 hashes.
...
- Evidence contract boundary: operator publication and public lifecycle reports must be sanitized structured artifacts bound directly through `evidence add-command --artifact-file`; summary-only assertions are insufficient for a release-readiness basis. T-0776 makes the binding byte-verifiable and fail-closed on mutation or same-key conflict, and invalidates the retained RC4 artifact for promotion; regenerate RC5 only after T-0776 closes.
