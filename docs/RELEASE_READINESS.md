...
- Current 0.5.0-rc.2 source status: T-0699 through T-0706 completed Init v1 implementation boundaries; T-0743 reduced the legacy status surface and decomposed close modules; T-0744 completed the initial release/document acceptance; T-0745 repaired post-freeze continuation, close-source, and installed lifecycle integrity; T-0746 corrected the shipped close/document/schema contract and made installed evidence reproducible; T-0747 through T-0750 completed final-head freeze, release-input binding, and the tracked GitHub Release note; T-0751 repaired the post-publish evidence path.
- Current 0.5.0-rc.2 readiness: T-0749 current-head release artifact, checksum/manifest, package/consumer smoke, clean-checkout smoke, strict release gate, release dry-run, publish dry-run, installed lifecycle acceptance, and closed-valid proof passed. Publication status: npm `0.5.0-rc.2` is published on `next`; GitHub Release `v0.5.0-rc.2` is public. Post-publish consumer recycle remains a separate capsule.
- Release artifact retention: the capsule retains the reduced public release-artifact report plus checksum and manifest metadata; the binary tarball remains disposable or external to avoid one committed `.tgz` per release capsule.
...
- Current version is `0.5.0-rc.2`.
- Current source version is `0.5.0-rc.2`.
- Current release target is source candidate `0.5.0-rc.2`; publication remains a separate approval-gated capsule.
- Previous release-candidate npm status: `hadara@0.5.0-rc.1` is published on dist-tag `next`; RC2 source readiness must be proven independently on current HEAD.
