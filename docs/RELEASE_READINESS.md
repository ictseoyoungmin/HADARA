...
- Current 0.4.5 stable source-preparation status: T-0585 defined docs-registry v3 and init cleanup; T-0586 through T-0592 implemented registry v3, mutation paths, project-authored defaults, migration cleanup, mutation hardening, and the brownfield adoption contract; T-0593 through T-0595 implemented brownfield detection, guarded adoption execute, and origin-aware doctor/idempotency cleanup; T-0596 dogfooded fresh profiles, governed lifecycle, brownfield execute, and fail-closed safety paths; T-0597 retargets source metadata, README release status, release notes, readiness docs, helper examples, and the GitHub stable release note artifact to `0.4.5`.
- Current 0.4.5 stable readiness: fresh `basic`, `standard`, and `governed` init all passed `init doctor`, `docs doctor --scope all`, and task status; a governed toy task closed with `task finalize --execute --auto`; brownfield adoption remained zero-write by default and wrote only after a matching plan hash; missing hash, mismatched hash, partial `.hadara`, and unsafe symlink cases failed closed.
...
- Current source version is `0.4.5`.
- Current release target is stable `0.4.5`.
- Current stable npm publish target is `0.4.5` on dist-tag `latest`.
- Current published stable npm release target is `0.4.4` until the operator publishes `0.4.5`.
