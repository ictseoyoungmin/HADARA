...
- Current version is `0.5.0-rc.4` for the published RC4 package; T-0766 reconciled fresh Init v1 scaffolds and protocol doctor now reports zero warnings across all three presets. T-0775 adds the evidence artifact-binding and operator publication-report contracts after that artifact was published.
...
- Evidence contract boundary: operator publication and public lifecycle reports must be sanitized structured artifacts bound directly through `evidence add-command --artifact-file`; summary-only assertions are insufficient for a release-readiness basis. T-0775 implements this contract and invalidates the retained RC4 artifact for promotion; regenerate RC5 after the source change.
