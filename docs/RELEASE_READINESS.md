...
- Current version is `0.3.0-rc.0`.
- Current source version is `0.3.0-rc.0`.
- Current published npm release candidate is `0.2.0-rc.3`.
- Previous published release candidate is `0.2.0-rc.2`.
...
- T-0296 prepares `hadara@0.3.0-rc.0` as a Phase 7 source candidate after command/help, lifecycle, docs registry, managed patch, and docs cleanup surfaces are implemented. This preparation does not perform npm publish, GitHub Release creation, Docker image build, registry mutation, or token loading.
...
- Current operator publish path: for the next npm release candidate, create a fresh publish-readiness capsule, commit the readiness state, authenticate with `npm login --registry=https://registry.npmjs.org`, then run that capsule's approval-gated manual helper and type `publish` only after the helper's own checks pass.
...
- Version policy: first release-candidate target was `0.1.0-rc.0`; the current source publish-candidate target is `0.3.0-rc.0`. Stable release targets remain approval-gated and must be set in a later release capsule.
