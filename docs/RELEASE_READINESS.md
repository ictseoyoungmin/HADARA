...
- Planned 0.5.0-rc.2 scope: rc.2 includes the current `0.5.0-rc.1` line, reviewer-fix capsules through T-0677, and Phase D through the end of the 0.5 DAG/status redesign before release readiness is recycled. Phase D starts at the deferred context-route resolver / registry stable-id work recorded in the T-0660/T-0661 handoff. Do not start rc.2 release readiness until structured continuation semantics fail closed for malformed and contradictory machine-readable HANDOFF values and the current validation baseline preserves source/release/publish/installed/remediation rollup evidence.
