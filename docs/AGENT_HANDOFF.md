...
| Latest Completed Task | T-0586 0.4.5 init upgrade project identity and gitkeep cleanup | Most recent completed capsule. |
| Active Task | None | Resume this capsule first. |
| Next Work | 0.4.5 docsRegistry v3 schema and read model | Structured continuation title; not operator prose. |
| Next Work State | candidate | Controls whether task creation guidance is emitted. |
| Operator Guidance | Implement staged capsule 2 from docs/specs/0.4.5/docs-registry-v3-and-init-cleanup.md: add v3 types, normalizer, migration read path, schema fixtures, and docs doctor diagnostics while keeping v1/v2 compatibility. | Human constraints; never used as a task title. |
