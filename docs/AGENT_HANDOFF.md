...
| Validation Baseline | Docker full check passed | Latest Docker temp-copy `npm run check` passed with 60 files and 437 tests; built CLI profile/task/docs smokes passed after refreshing `/workspace/dist`. |
...
| T-0156 Profile Drift Remediation Guide | Added `hadara protocol doctor --scope profile --json`, profile doc-set/metadata/Required Reading drift diagnostics, and manual remediation objects. | T-0156 evidence: focused protocol/profile tests passed with 2 files / 12 tests; full Docker check passed with 60 files / 437 tests; built CLI profile/task/docs smokes passed after refreshing `/workspace/dist`. |
...
|---|---|---|
| Focused protocol checks | Docker `npx vitest run tests/unit/protocol-consistency.test.ts tests/unit/protocol-cli.test.ts` passed with 2 files and 12 tests. | Covers profile-scope report shape, basic-to-governed remediation guidance, partial doc-set hints, JSON CLI output, and mutual exclusion behavior. |
| Full repository check | Docker temp-copy `npm run check` passed with 60 files and 437 tests. | Host dependencies were unavailable; Docker was used per SOP and changed files were synced into `/tmp/hadara` before the final check. |
| Built CLI smoke | Refreshed `/workspace/dist` from `/tmp/hadara/dist`; built CLI `protocol doctor --scope profile --json --project /workspace` returned `ok: true` with 0 issues; `protocol doctor --task T-0156 --json` returned `ok: true`; `protocol doctor --scope docs --json` returned `ok: true` with two historical warnings; `--task` plus `--scope profile` returned `CLI_OPTION_INVALID_VALUE`. | Run through `node /workspace/dist/cli/main.js ... --project /workspace`. |
