...
- TUI performance follow-up is required: current production TUI still builds a full synchronous aggregate for startup, full refresh, and selected-detail refresh. Direct measurement in `hadara-cli-test` against `/workspace` showed initial startup around 18.83s, full refresh around 22.35s, different-task detail entry around 24.24s, and full `createTuiReadModel('/workspace')` around 20.02s. The mockup felt faster because it used fixture/state-cache reads, async loading timers, parallel CLI reads, fast refresh over only status/tasks/active-run/resume, and selected-detail refresh over only task detail plus evidence.
...
| 20g | TUI async loading and read-model performance refactor | TBD | Restore mockup-level responsiveness by separating immediate shell/render state from expensive project read models. | Future: cache write/indexing tolerates missing/stale Task Board capsules as degraded warnings; startup opens immediately and keeps animation ticking while reads run off the main render path; initial fast model loads only status/tasks/active-run/resume plus selected detail; debt/release/tools/write preview are lazy or deferred; selected-task detail refresh reads only task detail/evidence; full refresh can run as an explicit background operation; measured startup/detail refresh times are recorded against `/workspace` and a synthetic large-task fixture. |
| 20h | TUI compatibility CLI adapter | TBD | Provide explicit `--source cli` compatibility with bounded HADARA CLI subprocesses while keeping shared services as the default data source. | Future: default TUI uses shared services, `--source cli` works for compatibility smoke, subprocess failures degrade to warnings, and no source-of-truth drift is introduced. |
