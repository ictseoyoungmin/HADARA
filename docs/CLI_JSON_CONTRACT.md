...
`hadara task close --task <id> --json` returns `hadara.task.close.v2`. It is the public primary close surface for ordinary clean work. The command performs an internal current-plan review, acquires transaction locks in the documented order, executes bounded lifecycle/status/evidence writes only after preflight passes, records readiness evidence when required, appends close proof last, and returns `ok:true` only after final audit reaches `closed-valid`. `hadara task close --task <id> --dry-run --json` is read-only and reports the same public schema with blocked/recovery guidance. `hadara task close --task <id> --execute --plan-hash <hash> --json` is the advanced externally reviewed form. Top-level `primaryNextAction` and `nextActions` are the agent-facing guidance; `source.finalize` is diagnostic/internal compatibility source metadata and may contain historical finalize step names.
...
| `task status --json` | `hadara.taskSelection.status.v2` | Read-only next-work selection. | Selection report was generated; not that a capsule exists. |
...
| `smoke package [--dry-run|--execute] [--from <tarball\|dir>] [--source-root <dir>] [--evidence-root <dir>] [--smoke-project-root <dir>] --json` | `hadara.packageSmoke.v1` | Dry-run by default. Reports source/evidence/smoke project root roles, packs from sourceRoot, attaches evidence to evidenceRoot, and executes installed smoke inside smokeProjectRoot without inherited source `HADARA_PROJECT_ROOT`. | Package smoke report was generated; execute returns `ok:true` only when package build/install/init/status smoke and cleanup pass. |
| `package recycle [--execute] [--package <specifier>] [--expected-version <version>] [--source-root <dir>] [--evidence-root <dir>] [--smoke-project-root <dir>] [--include-graph] --json` | `hadara.packageRecycle.v1` | Dry-run by default. Execute mode runs npm registry reads, isolated-prefix install, installed command-surface discovery, current-or-legacy installed CLI smokes inside smokeProjectRoot, and temp cleanup; it never publishes or mutates release targets. Broad context graph smoke is opt-in via `--include-graph`. | Recycle report was generated; execute returns `ok:true` only when registry metadata, installed version, command-surface/init/status/task-close/context smokes, and cleanup pass. |
