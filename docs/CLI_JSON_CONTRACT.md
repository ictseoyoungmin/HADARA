...
Removed handoff compatibility surface (0.4.1-rc.0): `hadara handoff stale-problems --json` returns `hadara.commandRemoved.v1` with `code: COMMAND_SURFACE_REMOVED`, `replacementCommand: "hadara status --json"`, and `diagnosticCommand: "hadara task status --json"`. The historical `hadara.handoff.staleProblems.v1` shape remains implementation history only.
