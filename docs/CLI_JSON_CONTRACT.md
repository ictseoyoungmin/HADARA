...
`hadara task lifecycle --task <id> --json` returns `hadara.task.lifecycle.v1`. It is a read-only normalized lifecycle phase report over the existing `task finish`, `task ready`, `task close`, and `task audit-close` read models. It reports `phase`, `checks`, `satisfied`, `blockers`, optional `repair`, and at most one `primaryNextAction`. It must not write files, append evidence, update handoff/state docs, run shell commands, or execute the reported next action.
...
`hadara package recycle [--execute] [--package <specifier>] [--expected-version <version>] --json` returns `hadara.packageRecycle.v1`. Dry-run mode is the default and plans the post-publish consumer-path recycle without registry access or install mutation. Execute mode explicitly runs npm registry metadata checks, isolated-prefix package install, installed `hadara version --json`, lifecycle help, disposable `hadara init`, task lifecycle, context graph/pack/slice, and session-start smokes, then removes the temporary workspace unless `--keep-temp` is set. Reports must not include raw npm logs, package contents, private paths, environment secrets, publish execution, or release mutation markers.
...
| `task lifecycle --task T-XXXX --json` | `hadara.task.lifecycle.v1` | Read-only normalized phase report. | Report generation succeeded for an existing task. |
...
| `package recycle [--execute] [--package <specifier>] [--expected-version <version>] --json` | `hadara.packageRecycle.v1` | Dry-run by default. Execute mode runs npm registry reads, isolated-prefix install, installed CLI smokes, and temp cleanup; it never publishes or mutates release targets. | Recycle report was generated; execute returns `ok:true` only when registry metadata, installed version, lifecycle/init/task/context/session smokes, and cleanup pass. |
