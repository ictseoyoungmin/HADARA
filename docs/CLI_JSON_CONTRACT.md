...
`hadara package recycle [--execute] [--package <specifier>] [--expected-version <version>] [--source-root <dir>] [--evidence-root <dir>] [--smoke-project-root <dir>] [--include-graph] --json` returns `hadara.packageRecycle.v1`. Dry-run mode is the default and plans the post-publish consumer-path recycle without registry access or install mutation. Reports expose `rootRoles.sourceRoot`, `rootRoles.evidenceRoot`, and `rootRoles.smokeProjectRoot`; compatibility `--project` aliases source and evidence roots with a warning. Execute mode explicitly runs npm registry metadata checks, isolated-prefix package install, installed `hadara version --json`, installed `hadara commands --json` command-surface discovery, lifecycle help, disposable `hadara init`, current `task status` selection/read-model smokes, public task-close dry-run smoke, context slice, and cleanup inside `smokeProjectRoot` without inherited source `HADARA_PROJECT_ROOT`. The installed package must expose the current task-status route; legacy lifecycle fallback is not part of the release recycle contract. Broad installed `context graph --json` smoke is opt-in via `--include-graph`. Reports include per-step timeout policy and identify slow timeout steps through `timeoutStepIds`; the default recycle timeout is 300 seconds unless `--timeout` is supplied. Reports must not include raw npm logs, package contents, private paths, environment secrets, publish execution, or release mutation markers.

Compatibility surfaces outside the current public command set do not have stable JSON response contracts. Consumer migration: read the close audit verdict from public `task close --task <id> --json`/`--dry-run --json` or from `task status --task <id> --detail full --json` `state.closeState`; use `task status --json` for next-work selection, `task status --task <id> --json` for selected-task reads, `validation run` or `evidence add-command` for evidence, `policy preflight-shell` for shell policy reads, `docs register/list/doctor/mark` for docs governance, repo-local `node --import tsx tools/dev-surfaces.ts smoke package` for package smoke validation, and `protocol remediate` for scaffold remediation.
...
| `evidence add-command --task T-XXXX ... [--outcome <outcome>] [--category <category>] [--artifact-file <path>] [--resolves <id>] [--supersedes <id>] [--idempotency-key <key>] --json` | `hadara.evidence.collect.v1` evidence append response | Writes command-log evidence; `--artifact-file` copies a sanitized UTF-8 report through the shared public artifact policy and persists its task-local path plus `sha256` and `byteLength` in v2 `artifacts[]` and legacy `evidencePath`. Artifact bytes participate in the v2 fingerprint; lint verifies bound bytes, and a same-key retry with different bytes fails closed. Explicit v2 metadata/tags are additive and result/outcome mismatches fail before append. | Evidence append succeeded or returned the existing keyed record only when bound artifact bytes match; artifact policy/integrity/idempotency failures are reported before evidence append. |
