...
`hadara task lifecycle --task <id> --json` returns `hadara.task.lifecycle.v1`. It is a compatibility read-only normalized lifecycle phase report over the existing `task finish`, `task ready`, `task close`, and `task audit-close` read models. New agent loops should prefer `task status --task <id> --json` for phase and next-action guidance. It reports `phase`, `checks`, `satisfied`, `blockers`, optional `repair`, and at most one `primaryNextAction`. It must not write files, append evidence, update handoff/state docs, run shell commands, or execute the reported next action.

`hadara task close-repair-plan --task <id> --json` returns `hadara.task.closeRepairPlan.v1`. It is a read-only close-proof diagnostic over `task audit-close` and `task close --json`. It classifies close state as `not-closed`, `closed-stale`, `closed-invalid`, `duplicate-close-proof`, `closed-valid`, or `unknown`, reports current/recorded hashes and causes, and returns exact repair next actions. It must not write files, append replacement close evidence, update handoff/state docs, run shell commands, or execute the reported next action.

`hadara task finalize --task <id> --json` returns `hadara.task.finalize.v1`. It is a read-only reviewed lifecycle plan over finish, ready, close, and audit-close. It reports ordered steps, step status, command, mode, write boundary, expected write paths, `alreadySatisfied`, and a stable `planHash` for review. `hadara task finalize --task <id> --execute --plan-hash <hash> --json` rechecks the current plan hash before writing, refuses missing or stale hashes, executes phases serially, stops on the first blocker, preserves underlying write boundaries, and returns `ok:true` only when final audit is `closed-valid`.

When audit evidence exists but close-source hashes drift after close, `task finalize` keeps the task repair-required: the audit step is required, `primaryNextAction` points to `hadara task close-repair-plan --task <id> --json`, and issues include `TASK_FINALIZE_CLOSE_SOURCE_DRIFT_GUIDANCE`. This is guidance only; close proof semantics are still enforced by `task close` and `task audit-close`.
...
`hadara package recycle [--execute] [--package <specifier>] [--expected-version <version>] [--include-graph] --json` returns `hadara.packageRecycle.v1`. Dry-run mode is the default and plans the post-publish consumer-path recycle without registry access or install mutation. Execute mode explicitly runs npm registry metadata checks, isolated-prefix package install, installed `hadara version --json`, lifecycle help, disposable `hadara init`, task lifecycle, session-start, task finalize dry-run, task-scoped context pack, context slice, and cleanup. Broad installed `context graph --json` smoke is opt-in via `--include-graph`. Reports must not include raw npm logs, package contents, private paths, environment secrets, publish execution, or release mutation markers.
...
`hadara task next --json` returns `hadara.task.next.v1`. It is a compatibility read-only recommendation report now superseded in the default loop by `hadara task status --json`. It prefers the actionable current next step in `docs/AGENT_HANDOFF.md`, then incomplete planned rows in `docs/DEVELOPMENT_SLICES.md`, then `docs/TASK_BOARD.md` fallback rows. Handoff meta-guidance that tells the operator to run or select with `task next` is not treated as an actionable work item. Task Board fallback prefers primary open statuses such as Draft/In Progress/Blocked before legacy Partial rows; Partial rows remain visible in `backlog` unless no stronger open row exists. Handoff-only recommendations may return `taskId: "TBD"` when the recommended work has no Task Capsule yet; consumers must inspect `sourceKind`, `createCommand`, `taskCapsulePresent`, and `backlog` instead of assuming every recommendation has a concrete task id. The report includes required-reading guidance plus a `createCommand` for capsule creation, and may expose non-primary legacy/open Task Board rows in additive `backlog` metadata. It must not create tasks, update project docs, append evidence, or infer completion.
...
| `task next --json` | `hadara.task.next.v1` | Read-only compatibility. | Recommendation report was generated. |
...
| `task close-repair-plan --task T-XXXX --json` | `hadara.task.closeRepairPlan.v1` | Read-only close repair classifier. | Report generation succeeded for an existing task. |
| `task finalize --task T-XXXX --json` | `hadara.task.finalize.v1` | Read-only reviewed lifecycle plan by default; guarded execute requires matching `--plan-hash`. | All finalize steps are satisfied, or guarded execute reaches `closed-valid`. |
...
| `session start [--task T-XXXX] [--include-code] [--budget <tokens>] [--max-items <count>] [--max-read-first <count>] [--live] --json` | `hadara.sessionStart.v1` | Read-only C5 startup packet; no evidence append, validation execution, document patch, cache write, source mutation, raw slice read, or hidden background work. Default mode may consume proven-fresh source-manifest, graph-core, and code-index cache read-only; when warm freshness cannot be proven without broad scanning, it returns a bounded no-live context-pack envelope. `--live` explicitly permits the underlying context-pack graph read. Guidance includes additive `primaryAction`, `whyThisNow`, `avoidForNow`, and `nextCommandArgs`; with a task id, the primary action is the read-only task lifecycle report. | Session start report was generated. Missing task errors return `ok:false`; default bounded fallback reports degradation explicitly in `issues`, `summary.degraded`, and `sourceSummary.degraded`, while warm hits report cache metadata such as `graph-core` or `graph-core+code-index`. |
...
| `package recycle [--execute] [--package <specifier>] [--expected-version <version>] [--include-graph] --json` | `hadara.packageRecycle.v1` | Dry-run by default. Execute mode runs npm registry reads, isolated-prefix install, installed CLI smokes, and temp cleanup; it never publishes or mutates release targets. Broad context graph smoke is opt-in via `--include-graph`. | Recycle report was generated; execute returns `ok:true` only when registry metadata, installed version, lifecycle/init/task/session/finalize/context smokes, and cleanup pass. |
