...
`hadara task close --task <id> --detail full --json` exposes `source.finalize` as internal diagnostic metadata for the underlying finish, ready, close, and audit-close engine used by `task close`. The internal schema remains `hadara.task.finalize.v1`, but there is no public `task finalize` route.

When audit evidence exists but close-source hashes drift after close, the internal finalize source keeps the task repair-required: the close step becomes required, internal diagnostic actions may mention recovery metadata, and issues include `TASK_FINALIZE_CLOSE_SOURCE_DRIFT_GUIDANCE`. Public callers should follow the top-level `task close` recovery action, finish close-source edits before executing the repair plan, and treat `source.finalize` actions as diagnostic internal metadata.
...
Fully removed compatibility surfaces: `task next`, `task show`, `task upgrade-scaffold`, `handoff stale-problems`, `handoff suggest`, `evidence collect`, `ops status`, `init register-doc`, `docs archive`, `task finish`, `task ready`, `task audit-close`, `task complete`, `task lifecycle`, `task finalize`, `write preflight`, `policy check-shell`, `harness replay`, `run`, `run scaffold`, `run-state show`, `run-state resume`, and `package smoke` are no longer routed as public commands and do not have a stable JSON response contract. Consumer migration: read the close audit verdict from public `task close --task <id> --json`/`--dry-run --json` or from `task status --task <id> --detail full --json` `state.closeState`; use `task status --json` for next-work selection, `task status --task <id> --json` for selected-task reads, `validation run` or `evidence add-command` for evidence, `policy preflight-shell` for shell policy reads, `status --json` for project status, `docs register/list/doctor/mark` for docs governance, `smoke package` for package smoke validation, and `protocol remediate` for scaffold remediation.
