...
`hadara task upgrade-scaffold --task <id> --json` was removed from the public CLI; use `protocol doctor` and `protocol remediate`. The historical `hadara.task.upgrade_scaffold.v1` report remains implementation history only.
...
Removed lifecycle compatibility surface (0.4.1-rc.0): `hadara task complete --task <id> --json` returns `hadara.commandRemoved.v1` with `code: TASK_LIFECYCLE_COMMAND_REMOVED` and `replacementCommand: "hadara task status --task <task-id> --json"`. The historical `hadara.task.complete_flow.v1` shape remains implementation history only.

Removed lifecycle compatibility surface (0.4.1-rc.0): `hadara task lifecycle --task <id> --json` returns `hadara.commandRemoved.v1` with `code: TASK_LIFECYCLE_COMMAND_REMOVED` and `replacementCommand: "hadara task status --task <task-id> --json"`. The historical `hadara.task.lifecycle.v1` shape remains implementation history only.
...
Removed handoff compatibility surface (0.4.1-rc.0): `hadara handoff suggest --task <id> --json` returns `hadara.commandRemoved.v1` with `code: COMMAND_SURFACE_REMOVED`, `replacementCommand: "hadara task status --task <task-id> --json"`, and `diagnosticCommand: "hadara task finalize --task <task-id> --json"`. Shared handoff edits are manual reviewed docs work before finalize. The historical `hadara.handoff.suggestion.v1` shape remains implementation history only.

Removed handoff compatibility surface (0.4.1-rc.0): `hadara handoff stale-problems --json` returns `hadara.commandRemoved.v1` with `code: COMMAND_SURFACE_REMOVED`, `replacementCommand: "hadara state verify --json"`, and `diagnosticCommand: "hadara task status --json"`. The historical `hadara.handoff.staleProblems.v1` shape remains implementation history only.
...
Removed command surface (0.4.1-rc.0): `task next`, `task show`, `task upgrade-scaffold`, `task finish`, `task ready`, `task close`, `task audit-close`, `task complete`, `task lifecycle`, `handoff suggest`, `handoff stale-problems`, `evidence collect`, `write preflight`, `policy check-shell`, `ops status`, `init register-doc`, `docs archive`, `harness replay`, `run`, `run scaffold`, `run-state show`, `run-state resume`, and `package smoke` return `hadara.commandRemoved.v1` stubs with a machine-readable `replacementCommand` (stub kept for at least one minor release). Low-level lifecycle removals use `code: TASK_LIFECYCLE_COMMAND_REMOVED`; other compatibility removals use `code: COMMAND_SURFACE_REMOVED`. Consumer migration: read the close audit verdict from `task finalize --task <id> --json` `state` or from `task status --task <id> --detail full --json` `state.closeState`; use `task status --json` for next-work selection, `task status --task <id> --json` for selected-task reads, `validation run` or `evidence add-command` for evidence, `policy preflight-shell` for shell policy reads, `status --json` for project status, `docs register/list/doctor/mark` for docs governance, `smoke package` for package smoke validation, and `protocol remediate` for scaffold remediation. The schema descriptions below document internal report shapes that finalize and shared services may still produce.
...
The historical `hadara.task.next.v1` report is now an internal compatibility projection consumed by `task status --json`; public `hadara task next --json` returns `hadara.commandRemoved.v1` with `replacementCommand: "hadara task status --json"`.
...
The historical active-run projection/resume reports remain internal service and MCP read-model shapes. Public `hadara run-state show/resume` now returns `hadara.commandRemoved.v1` and points to `hadara status --json`.
