...
`hadara dev docker-check [--focused <test...>] [--full] [--sync-dist] --json` returns `hadara.dev.docker_check.v1`. It is the official Phase 6 Docker validation wrapper for reproducible temp-copy validation. It runs Docker as an external subprocess, creates a run-scoped temp workspace with `.git`, `.hadara`, `node_modules`, and `dist` excluded, runs `npm ci`, then focused tests, a full check, or both. `--sync-dist` must be explicit before the wrapper copies Docker-built `dist` back to the workspace; reports include dist sync before/after hashes and conflict metadata. JSON output omits raw subprocess logs, private paths, and environment secrets, and includes an evidence-ready summary.
