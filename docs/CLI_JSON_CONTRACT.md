...
`hadara task next --json` returns `hadara.task.next.v1`. It is a read-only recommendation report that prefers the actionable current next step in `docs/AGENT_HANDOFF.md`, then incomplete planned rows in `docs/DEVELOPMENT_SLICES.md`, then `docs/TASK_BOARD.md` fallback rows. The report includes required-reading guidance plus a `createCommand` when the recommended work has no Task Capsule yet, and may expose non-primary legacy/open Task Board rows in additive `backlog` metadata. It must not create tasks, update project docs, append evidence, or infer completion.
