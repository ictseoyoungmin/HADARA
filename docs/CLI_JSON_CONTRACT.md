...
`hadara handoff suggest --task <id> --json` returns `hadara.handoff.suggestion.v1`. It is a read-only shared-doc suggestion report for `docs/AGENT_HANDOFF.md`; it includes default actor context, target before-hash, `writeBoundary: "shared-doc"`, `recommendedActorRole: "coordinator"`, task snapshot metadata, section fragments, and issues. It must not write the handoff, Task Capsule files, project state docs, evidence, or Task Board rows. `--execute` is intentionally unsupported and returns the same schema with `ok:false` and `HANDOFF_SUGGEST_EXECUTE_UNSUPPORTED`.

`hadara dev docker-check [--focused <test...>] [--full] [--sync-dist] --json` returns `hadara.dev.docker_check.v1`. It is the official Phase 6 Docker validation wrapper for reproducible temp-copy validation. It runs Docker as an external subprocess, creates a run-scoped temp workspace with `.git`, `.hadara`, `node_modules`, and `dist` excluded, runs `npm ci`, then focused tests, a full check, or both. `--sync-dist` must be explicit before the wrapper copies Docker-built `dist` back to the workspace; reports include dist sync before/after hashes and conflict metadata. T-0261 clarifies mutation fields: compatibility `execution.projectMutation:false` means no project source mutation, `execution.projectSourceMutation:false` says that directly, and `execution.outputMutation:true` means workspace output such as `dist` was changed. JSON output omits raw subprocess logs, private paths, and environment secrets, and includes an evidence-ready summary.
...
| `dev docker-check [--focused <test...>] [--full] [--sync-dist] --json` | `hadara.dev.docker_check.v1` | Runs Docker subprocess; writes workspace `dist` only when `--sync-dist` is explicit. | Requested Docker validation completed without blocking issues. |
