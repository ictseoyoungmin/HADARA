...
| `context graph [--task T-XXXX] --json` | `hadara.contextGraph.v1` | Read-only projection; no evidence append, validation execution, document patch, cache write, or local-state mutation. | Context graph report was generated; warning-level degradation is reported in `summary.degraded`/`issues`, while graph or state projection errors return `ok:false`. |
| Internal code index read model | `hadara.codeIndex.v1` | Read-only projection fixture only; T-0353 does not add a public CLI, cache write, graph integration, source mutation, import/export extraction, symbol extraction, command hints, or test relation heuristics. | Schema-valid code index reports can be produced by internal helpers; public `context graph --include-code` or dedicated code commands remain future C2 work. |
