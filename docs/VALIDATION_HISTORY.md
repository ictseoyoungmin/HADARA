...
- T-0142 package metadata transition evidence recorded fresh `hadara.packageSmoke.v1` public package-smoke evidence for version `0.1.0-rc.0`; authoritative details remain in `tasks/T-0142-package-metadata-transition-plan/evidence.jsonl`.
- T-0142 release-candidate validation: focused Docker tests for operational debt, release dry-run, and release publish passed with 3 files and 30 tests; Docker full `npm run check` passed with 57 files and 404 tests; strict release gate, release dry-run, release publish dry-run, and done-level harness returned `ok: true` with no publish/GitHub/Docker/registry mutation.
- T-0143 manual RC npm publish: container `npm run check` passed with 57 files and 404 tests, package-smoke and clean-checkout evidence attached, release artifact evidence attached, strict release gate/release dry-run/release publish dry-run passed, npm tarball dry-run passed, and `hadara@0.1.0-rc.0` was published to npm. Registry verification returned `https://registry.npmjs.org/hadara/-/hadara-0.1.0-rc.0.tgz`.
