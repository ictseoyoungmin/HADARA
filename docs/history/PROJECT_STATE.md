...
| Latest Completed Task | T-0735 Close plan final contract cleanup |
...
T-0728 strengthens the deterministic close recovery contract after reviewer P1 feedback: operation markers now persist close source hash, write set hash, expected guarded writes, intended final state, final source/proof intent, and stricter phases including `proof-pending`; malformed markers fail closed with zero lifecycle/evidence writes; `executedFileWrites` counts task-local target files separately from mutating transaction steps; and task-close v3 schema/tests cover the new contract. T-0729 closes the remaining reviewer Px gaps: schema-valid mismatched markers no longer get overwritten, schema-invalid JSON markers fail closed, `closeSourceHash` is source-backed with a separate plan fingerprint, expected writes describe readiness/proof appends, proof-pending does not regress through progress events, durable marker writes are journal-boundary writes, and the v3 schema now conditionally requires task-local before/after hashes. T-0730 fixes the Docker sync dist guard path namespace bug and unignores `docs/specs/0.5.0-rc2/**` so CI sees the current rc2 spec line. T-0731 implements recovery hardening for actual task-local expected-write hash reconciliation, source-drift fail-closed continuation, proof-pending phase preservation, stricter runtime marker shape validation, detailed recovery reports, and active rc2 spec registry discovery. T-0732 completes follow-up reconciliation hardening. T-0733 closes the remaining reviewer P1/P2 task-close correctness and contract gaps: proof append performs actual-state revalidation, marker hash consistency is mandatory, missing-target symlink confinement is hardened, proof-pending recovery stays phase-accurate, recovery reporting avoids fabricated completed/pending detail, close-basis/final-source hashes are separated, stale current-state projection is cleaned up, and public close-plan terminology no longer exposes the legacy bookkeeping step/domain. T-0734 completes marker-bound proof append and removes public `sync` lifecycle steps. T-0735 completes the final task-close contract cleanup: public close plans directly own `writes` and `writeSetHash` without a guarded-write sub-report identity, proof append requires a transaction marker guard, proof-boundary task disappearance is structured fail-closed, operation basis and final source hashes are distinct, and generated workflow guidance describes the proof-last order.
