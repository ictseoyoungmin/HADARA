...
Phase 6.1 added optional actor CLI input for then-existing workflow-compression surfaces. The low-level lifecycle and handoff suggestion surfaces were later removed behind structured redirect stubs; `dev docker-check` remains the current external-subprocess validation wrapper with actor metadata. Future plan/idempotency work should use `--idempotency-key` where a command accepts reviewed write plans.
...
| `hadara task next --json` | Removed in 0.4.1-rc.0; returns a `hadara.commandRemoved.v1` stub pointing at `task status --json`. | Stub, no writes. | No. | Redirect report generated. | Task-style failures use 6. |
...
| `hadara task lifecycle --task T-XXXX --json` | Removed in 0.4.1-rc.0; returns a `hadara.commandRemoved.v1` stub pointing at `task status --task T-XXXX --json`. | Stub, no writes. | No. | Redirect report generated. | Task-style failures use 6. |
...
| `hadara handoff suggest --task T-XXXX --json` | Removed in 0.4.1-rc.0; shared handoff edits are manual and status/finalize own diagnostics. | Stub, no writes. | No. | Redirect report generated. | Task-style failures use 6. |
...
| `hadara task audit-close ...` | Removed in 0.4.1-rc.0 (FD-013); audit verdicts are available through `task finalize --json` and `task status --detail full --json`. | Stub, no writes. | No. | Redirect report generated. | Task-style failures use 6. |
...
- `task complete`, `task next`, and `task lifecycle` are removed redirect stubs. Use `task status --json` for next-work selection and `task status --task T-XXXX --json` for phase/next-action guidance.
...
- `handoff suggest` is a removed redirect stub. No current CLI command writes or generates handoff fragments; edit `docs/AGENT_HANDOFF.md` deliberately before finalize when shared state changes.
