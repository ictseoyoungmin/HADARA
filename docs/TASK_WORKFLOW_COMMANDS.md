...
| `hadara status baseline promote --release <version> --task <task-id> --summary <text> --evidence <ev:id>[,<ev:id>] --json` | Preview promotion of a reviewed validation set into the structured current-state validation baseline and optional current release. | Dry-run. | No. | Promotion plan was generated, evidence ids exist and are passed, and a `planHash` was returned. | Task-style failures use 6. |
| `hadara status baseline promote --release <version> --task <task-id> --summary <text> --evidence <ev:id>[,<ev:id>] --execute --plan-hash <hash> --json` | Apply the reviewed validation baseline/current release to `.hadara/state/current.json` and managed PROJECT_STATE/HANDOFF projections. | Execute. | Yes, managed current-state bundle only. | The reviewed plan hash matched and baseline promotion writes applied without drift/conflict. | Task-style failures use 6. |
