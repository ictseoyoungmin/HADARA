...
| `hadara status baseline promote --summary <text> --evidence <ev:id>[,<ev:id>] --json` | Preview promotion of a reviewed validation set into the structured current-state validation baseline. | Dry-run. | No. | Promotion plan was generated and has no validation issues. | Task-style failures use 6. |
| `hadara status baseline promote --summary <text> --evidence <ev:id>[,<ev:id>] --execute --json` | Apply the reviewed validation baseline to `.hadara/state/current.json` and managed PROJECT_STATE/HANDOFF projections. | Execute. | Yes, managed current-state bundle only. | Baseline promotion writes applied without drift/conflict. | Task-style failures use 6. |
