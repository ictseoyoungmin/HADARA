...
- `task complete` and `task lifecycle` remain removed redirect stubs for lifecycle migration. `task next` has been fully removed from public routing; use `task status --json` for next-work selection and `task status --task T-XXXX --json` for phase/next-action guidance.
