...
`task close` includes done-level Task Capsule validation through its internal ready/close steps. Use `task status --detail full` and `task close --dry-run --json` to inspect capsule format, status-history, acceptance, evidence, or handoff blockers; `task close --json` validates the virtual post-write state before mutation, applies the reviewed guarded writes, revalidates the actual filesystem state, records readiness evidence and close proof, then performs the final audit.
...
Before task close, finish Task Capsule docs, acceptance/tests/handoff notes, and evidence summaries. Task Board close-plan guarded writes are projected by close. Optional shared prose remains human-owned, and Development Slices applies only when it already links the selected task. `HANDOFF.md` may be updated during the task as a work-in-progress checkpoint. Before close, reread it and convert it into close-time handoff: move all same-task close prerequisites out of `Pre-Close Operator Action`, and keep only guidance that remains true after this task closes in `Post-Close Continuation`. Close does not rewrite worker-owned handoff prose; a Done capsule must therefore not retain a pre-close instruction as its only current continuation. After `task close --json` reaches close proof, changing close-source documents requires rerunning task close.
...
T-0257 added read-only `handoff suggest` reports for coordinator-reviewed shared handoff updates. T-0506 removed that public helper after dogfood showed stale fragments and duplicated task status/close guidance. Shared handoff edits are now manual reviewed docs work before task close.
...
| `hadara evidence add-command --task T-XXXX --summary "..." --result passed [--outcome <outcome>] [--category <category>] [--artifact-file <path>] [--resolves <id>] [--supersedes <id>] [--idempotency-key <key>] --json` | Record command-log evidence supplied by the operator; optionally bind a sanitized UTF-8 report with exact `sha256`/`byteLength` into canonical `artifacts[]`. | Write command. | Yes, appends capsule evidence and copies the artifact through the shared public-artifact policy unless an explicit idempotency key already exists; bound bytes are included in the fingerprint. | Evidence append succeeded or returned the existing keyed record only when artifact bytes match; policy, integrity, or idempotency conflicts fail before append. | Evidence/task-style failures use 6. |

## Non-Overlap Rules

- `task status --json` chooses work when no task is selected; it does not create a capsule or infer completion. Handoff-only recommendations may use `taskId: TBD`; consumers must inspect `sourceKind`, `taskCapsulePresent`, `createCommand`, and `backlog`.
...
- `handoff suggest` is fully removed from public routing. Task close updates Task Board state and task-local close sources only; no command invents user-authored handoff prose.
...
- Readiness diagnostics live in `task close --dry-run` and `task status --detail full`.
...
- `evidence add-command` records an operator-supplied command result; it does not execute shell commands or capture stdout/stderr. `--artifact-file <path>` is the structural binding boundary for a sanitized UTF-8 command/release report: the shared writer copies it into the task capsule, rejects binary or secret-bearing content before append, computes exact `sha256` and `byteLength`, includes those fields in the v2 fingerprint, and persists the task-local path in v2 `artifacts[]` plus the legacy `evidencePath` compatibility field. Evidence lint treats missing or mismatched bound bytes as errors; historical refs without byte metadata remain readable for compatibility. `--category` and `--outcome` set persisted v2 metadata explicitly, while `--result` remains the legacy-compatible command result. When both are supplied, `--result` must match `--outcome` for `passed`, `failed`, `blocked`, and `unknown`; `recorded` and `not-applicable` require `--result unknown` or no explicit `--result`. Mismatches fail with `EVIDENCE_RESULT_OUTCOME_MISMATCH`. `--resolves` and `--supersedes` append exact v2 resolution tags, and only later `passed` or `recorded` evidence can resolve earlier failed evidence through those tags. `--idempotency-key` is optional; when supplied, same-key repeats return the existing record only when incoming bound artifact bytes match; a different or legacy artifact fails with `EVIDENCE_IDEMPOTENCY_ARTIFACT_CONFLICT`. Evidence append responses include `evidence.appendLock`; if `contended` is true, another process held the task-scoped append lock before this write.
- `validation run` adds a stable check key to its evidence. Its canonical `status` is a controlled `Passed|Failed|Blocked` token and `detail` is bounded explanatory prose; never infer status by parsing detail. `execution.failureClass` is also controlled: `assertion` means the validation command started and returned non-zero, `timeout` means its deadline expired, and `environment-setup` covers launch, permission, or missing-command preparation failures; `none` means success. Low-level `failureKind` remains available for diagnostics. New Task Capsules use `Check | Gate | Status | Detail | Evidence`, while legacy Result-only tables remain compatible. Non-JSON output separates child command metadata from HADARA evidence recording, so logs can distinguish the executed command result from the evidence append/TASK.md sync summary. When a later attempt passes with the same check identity—the same check label and exact command argv—it automatically adds `resolves:<id>` tags for earlier unresolved failed or blocked attempts from that identity. Reusing a label with a different command does not resolve the earlier attempt. Use explicit `--resolves` only for cross-check, non-validation, or non-obvious repair relationships.
...
- The internal close-proof step may append only close evidence. Earlier finish close-plan guarded writes in the same public transaction owns Task Board projection; neither phase invents broad prose or creates optional state documents.
...
`task close` never creates optional shared documents or invents broad prose. It updates Task Board state and treats Development Slices as applicable only when the selected task is explicitly linked. Product narrative remains human-owned.
