...
| FD-012 | Slices state prototype | Prototype `.hadara/state/slices.json` as canonical slice state with `docs/DEVELOPMENT_SLICES.md` as a fully generated projection. | First bounded state-first experiment; gates the 0.5 state-first RFC (`docs/specs/0.5/state-first/RFC.md` section 9) and fixes the current `rows: 0` DEVELOPMENT_SLICES parsing failure. | Done in T-0500 (AC-6 dogfood metric partial, see T-0500 TASK.md RF-1/RF-2) |
| FD-013 | Lifecycle surface | Remove the low-level lifecycle command surface (`task finish`, `task ready`, `task close`, `task audit-close`, `task complete`, `task lifecycle`) after FD-010 lands. | Once `--auto` covers low-ceremony closes, standalone write steps outside the finalize snapshot guard are unprotected paths and surface-drift debt; read-only ones duplicate `task status --detail full`. Recovery flows must complete through finalize alone (see `docs/specs/0.4.1/rc0-scope.md` item 6). | Done in T-0500 |
...
| Slices state prototype and lifecycle surface removal | Completed in T-0500 | Implements FD-012/FD-013: `.hadara/state/slices.json` + generated `DEVELOPMENT_SLICES.md` with an ownership-contract drift guard, and `hadara.commandRemoved.v1` deprecation stubs for the six removed lifecycle commands. FD-012's AC-6 dogfood-count metric is partial (infrastructure proven, not yet exercised over 2 real capsules); see T-0500 TASK.md RF-1/RF-2 for the follow-up. |

## Review Trigger

Before preparing `0.4.1-rc.0`, open capsules for FD-001 through FD-013 or explicitly reclassify any item as deferred with evidence. FD-011/FD-012/FD-013 carried capsule-level contracts (acceptance criteria and evidence plans) in `docs/specs/0.4.1/rc0-scope.md`; all are now closed via T-0497/T-0499/T-0500. The `docs/specs/0.4.1/rc0-scope.md` 릴리스 판정 section still gates actual `0.4.1-rc.0` tagging on FD-012's AC-6 resolution (Done or explicit exclusion with evidence).
