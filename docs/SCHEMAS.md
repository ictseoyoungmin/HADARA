...
| `hadara.task.close.v3` | `src/schemas/task-close-v3.schema.json` | fixture | Documents the public task close transaction report, including close state, transaction mode, plan hash, lock order, lock diagnostics, operation/recovery state, write summary, recovery guidance, and close-plan source metadata. |
