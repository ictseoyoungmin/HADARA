...
| `hadara.contextPack.v1` | `src/schemas/context-pack.schema.json` | fixture | Documents the C3 bounded context pack read model over C1/C2 graph output, including ranking buckets, validation/write-boundary hints, slice candidates, source summary, and cache metadata; public CLI exists, while raw text retrieval is handled by `hadara.contextSlice.v1` and persistent cache writes remain future work. |
| `hadara.contextSlice.v1` | `src/schemas/context-slice.schema.json` | fixture | Documents the C4 read-only raw context slice report over explicit project files and context-pack candidates, including line bounds, original text, source hash, strategy, confidence, bounded summary, and safety issues. |
