...
Schema layer status: planning and fixture registration, with limited active-run, write-preflight, install-plan, feature-smoke, package-smoke, clean-checkout smoke, release-artifact, release evidence summary, protocol consistency/remediation/migration, evidence lint, task create, task close/audit-close, task complete flow, handoff suggestion, dev Docker check, task ready, task finish, task next, task upgrade-scaffold, task workbench, runtime version, state projection, dashboard bootstrap, dashboard core, dashboard task-detail, dashboard timeline, Phase 6 common actor/plan/next-action validation, Phase 7.1 command registry/help fixtures, Phase 7.2 lifecycle guide/portfolio audit fixtures, Phase 7.3 document registry/docs doctor fixtures, Phase 7.4 managed patch plan fixtures, and Phase 7.5 docs cleanup fixtures in focused contract tests.
...
T-0361 registers `hadara.contextPack.v1` for the 0.3.3 C3 Context Pack foundation. This fixture documents bounded task-scoped context pack reports with `readFirst`, `readIfNeeded`, `doNotReadByDefault`, validation suggestions, write-boundary hints, slice candidates, known problems, state projection summary, source summary, cache metadata, and issue taxonomy. T-0362 exposes the read-only public `hadara context pack --task T-XXXX --json` CLI over the current graph; actual C4 slicing and persistent C6 cache writes remain deferred.

T-0363 registers `hadara.context.sourceManifest.v1` for the 0.3.3 C6.1 source manifest foundation. This fixture documents metadata-first project-relative source discovery, source kind and extractor-key classifications, budget/degraded output, manifest hashes, optional carried content hashes, and comparison inputs for future cache invalidation. The first implementation is internal and read-only; cache file writes, cache status, graph/code-index integration, and C4 slicing remain deferred.
...
| `hadara.contextPack.v1` | `src/schemas/context-pack.schema.json` | fixture | Documents the C3 bounded context pack read model over C1/C2 graph output, including ranking buckets, validation/write-boundary hints, slice candidates, source summary, and cache metadata; public CLI exists, while C4 slicing and persistent cache writes remain future work. |
