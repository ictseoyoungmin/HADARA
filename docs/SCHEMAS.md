...
Schema layer status: planning and fixture registration, with limited active-run, write-preflight, install-plan, feature-smoke, package-smoke, clean-checkout smoke, release-artifact, release evidence summary, protocol consistency/remediation, evidence lint, task close/audit-close, task complete flow, handoff suggestion, task ready, task finish, task next, task upgrade-scaffold, task workbench, runtime version, dashboard bootstrap, dashboard core, dashboard task-detail, dashboard timeline, and Phase 6 common actor/plan/next-action validation in focused contract tests.
