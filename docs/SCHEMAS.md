...
Schema layer status: planning and fixture registration, with limited active-run, write-preflight, install-plan, feature-smoke, package-smoke, package-recycle, clean-checkout smoke, release-artifact, release closeout, release evidence summary, protocol consistency/remediation/migration, evidence lint/list, task create, task close/audit-close, task close transaction v2, task finalize plan/execute, dev Docker check, task ready, task finish, task-selection status projection, task workbench, runtime version, state projection, Phase 6 common actor/plan/next-action validation, Phase 7.1 command registry/help fixtures, Phase 7.2 lifecycle guide/portfolio audit fixtures, Phase 7.3 document registry/docs doctor fixtures, Phase 7.4 managed patch plan fixtures, Phase 7.5 docs cleanup fixtures, and 0.4 docs registry/register/read-map/inbox/complete-spec fixtures in focused contract tests.

T-0079 added fixture registration only. T-0092 added a lightweight runtime validation API for `hadara.active_run.projection.v1` and `hadara.active_run.resume.v1` because those read models are backed by mutable local state. T-0098 registers and validates `hadara.write.preflight.v1` reports before returning CLI write-boundary preflight output. T-0129 validates `hadara.install.plan.v1` before returning installer dry-run plans. T-0131 validates `hadara.featureSmoke.v1` before returning reduced core feature smoke reports over service/read-model surfaces. T-0132 registers `hadara.packageSmoke.v1` and validates deterministic package-smoke report fixtures before package-smoke dry-run or execution commands exist. T-0137 registers and validates `hadara.releaseArtifact.v1` before returning release artifact builder reports. T-0138 registers `hadara.smokeEvidenceSummary.v1` and `hadara.releaseArtifact.manifest.v1` before release gates read existing evidence records or optional reduced summary artifacts. T-0140 registers and validates `hadara.releaseDryRun.v1` before returning final release dry-run reports. T-0141 registers `hadara.releasePublish.v1` before returning approval-gated publish/deploy readiness reports. T-0159 registers `hadara.protocol.consistency.v1` and `hadara.protocol.remediation.v1` and validates focused service/CLI contract reports, including remediation action hash/existence fields. T-0165 registers `hadara.evidence.lint.v1` for early evidence index lint reports. T-0166 registers `hadara.task.close.v1` for close plan reports with loop-boundary metadata. T-0168 registers `hadara.task.ready.v1` for read-only readiness preflight reports. T-0170 registers `hadara.task.audit_close.v1` and clarifies task-close report/source hash semantics. T-0173 registers `hadara.task.workbench.v1` for the read-only Phase 3 task operator console. T-0178 registers `hadara.runtime.version.v1` for read-only CLI origin diagnostics. T-0180 registers `hadara.task.finish.v1` for dry-run-first bounded task finish/status sync reports. T-0529 renamed the internal next-work projection fixture to `hadara.task.selection.v1` so it no longer shares the removed public `task next` command name. Broad schema validation remains deferred.

Historical dashboard schema fixtures were removed with the browser surface in T-0691 on July 23, 2026. Current read-model schema work remains focused on task, status, context, release, and governance surfaces. T-0652 registers `hadara.task.close.v2` as the public 0.5 close transaction report over the existing finalize engine, making `task close` the ordinary one-call close route while keeping finalize as a compatibility/debug schema.

T-0253 registers Phase 6 common context fixtures: `hadara.actor_context.v1`, `hadara.plan_context.v1`, and `hadara.next_action.v1`. These are vocabulary/metadata fixtures for future workflow-compression reports, not new command execution surfaces. T-0254 adopts the common actor and next-action vocabulary in task finish, ready, close, and audit-close schemas as additive fields while preserving existing schema ids. T-0255/T-0393/T-0394/T-0257 previously registered completion-flow, lifecycle, close-repair, and handoff-suggestion convenience schemas; T-0534 removed those dead current fixtures after the public command surfaces were removed and no runtime consumer remained. T-0396 registered `hadara.task.finalize.v1` for reviewed lifecycle plans with plan hashes and step write boundaries; T-0397 extends the same fixture to cover guarded execute reports with plan-hash matching, serial execution metadata, stop-on-blocker semantics, and final audit proof. T-0399 adds lazy evaluated/skipped report diagnostics and weak-evidence repair hints to the same finalize fixture without changing its schema id or proof boundaries. T-0256 adds close evidence idempotency/supersedes metadata to `hadara.task.close.v1` and `hadara.task.audit_close.v1` without changing schema ids. T-0410 registers `hadara.releaseCloseout.v1` for read-only release closeout planning across release docs, shared state docs, and selected release capsule files. T-0413 registers `hadara.packageRecycle.v1` for dry-run-first installed-package recycle reports over npm registry metadata, isolated-prefix install, installed CLI smokes, and cleanup. T-0258 registers `hadara.dev.docker_check.v1` for the Docker temp-copy validation wrapper with focused/full modes, explicit dist sync metadata, redacted output, and evidence-ready summaries. T-0259 registers `hadara.task.create.v1` for task creation reports with optional template metadata.
...
T-0293 registers `hadara.docs.list.v1`, `hadara.docs.doctor.v1`, and `hadara.docs.explain.v1` for Phase 7.3 document registry projections. These fixtures document read-only document classification, registry drift diagnostics, and per-document read/update guidance from `.hadara/docs-registry.json`. T-0430 registers `hadara.docs.register.v1` for the 0.4 registry-first `hadara docs register` write surface, which updates `.hadara/docs-registry.json` without appending rows to AGENTS, context, workflow, or legacy SOP prose. T-0431 registers `hadara.docs.readMap.v1` and `hadara.docs.inbox.v1` for registry-backed read guidance and docs attention items. T-0495 registers `hadara.docs.completeSpec.v1` for the guarded `docs complete-spec` lifecycle mutation. T-0587/T-0588 register `hadara.docsRegistry.v3` and `hadara.docs.registryMutation.v1` for the v3 desired-state registry and guarded update/archive/supersede/unregister/render reports. `docs.mark-drift` remains a registry-visible planned surface only and intentionally has no schema fixture until a future capsule implements its report and mutation semantics.
...
T-0295 registers `hadara.docs.mark.v1` and `hadara.docs.requiredReading.v1` for Phase 7.5 document cleanup operations. These fixtures document registry-only status transitions and effective required-reading exclusions for historical, superseded, and archived documents. T-0308 adds additive semantic `tier` metadata to `hadara.docs.requiredReading.v1` entries with values `current-state`, `task-work`, `conditional-reference`, `historical`, and `excluded`.
...
T-0378 registered `hadara.sessionStart.v1` for the bounded C5 Session Start MVP. The schema remains historical implementation documentation after 0.5.0 removed public `session start` routing. Current public ingress uses `hadara status --json`, selected-task guidance uses `hadara task status --task T-XXXX --json`, and file-routing context uses `hadara context pack --task T-XXXX --json`.
...
| `hadara.sessionStart.v1` | `src/schemas/session-start.schema.json` | fixture | Historical implementation fixture for the removed public `session start` route; use status/task status/context pack for current public ingress. |
...
| `hadara.docs.inbox.v1` | `src/schemas/docs-inbox.schema.json` | fixture | Documents `hadara docs inbox --json` registry attention items. |
...
| `hadara.docs.register.v1` | `src/schemas/docs-register.schema.json` | fixture | Documents `hadara docs register --json` registry-first document registration reports. |
| `hadara.docs.registryMutation.v1` | `src/schemas/docs-registry-mutation.schema.json` | fixture | Documents guarded docs registry mutation reports for update, archive, supersede, unregister, and render. |
...
| `hadara.docsRegistry.v3` | `src/schemas/docs-registry-v3.schema.json` | fixture | Documents the v3 docs registry desired-state file shape with project identity and explicit document origins. |
...
| `hadara.task.close.v2` | `src/schemas/task-close-v2.schema.json` | fixture | Documents the public 0.5 task close transaction report, including close state, transaction mode, plan hash, lock order, lock diagnostics, operation/recovery state, write summary, recovery guidance, and embedded finalize-source metadata. |
...
| `hadara.task.selection.v1` | `src/schemas/task-selection.schema.json` | fixture | Documents the internal task-selection projection consumed by `task status --json` from structured current state, handoff, slices, Task Board fallback, and backlog state. |
...
| `hadara.packageRecycle.v1` | `src/schemas/package-recycle.schema.json` | fixture | Documents dry-run-first installed-package recycle reports over npm registry metadata, isolated-prefix package install, installed command-surface discovery, current-or-legacy installed CLI task/init/context/session smokes, cleanup, and no publish/release mutation markers. |
...
- No shell execution, provider calls, separate browser UI APIs, or release/package execution is introduced by this registry.
